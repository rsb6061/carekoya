...
async function importLegacyCohort(request:Request,env:Env){
  if(!env.DB) return json({ok:false,error:"Database not configured"},{status:503});
  const data=await readJson(request);
  if(clean(data?.migration,80)!=="carekoya-legacy-47") return json({ok:false,error:"Invalid migration marker"},{status:400});
  ...
  for(const profile of profiles){
    const legacyId=clean(profile.id,120);
    if(!legacyId) return json({ok:false,error:"Legacy profile missing id"},{status:400});
    const hash=await sha256Hex(legacyId);
    if(!LEGACY_ID_HASHES.has(hash)||seen.has(hash)) return json({ok:false,error:"Legacy cohort fingerprint mismatch"},{status:403});
    seen.add(hash);
  }
  if(seen.size!==LEGACY_ID_HASHES.size) return json({ok:false,error:"Legacy cohort incomplete"},{status:403});
  ...
  for(const p of profiles){
    ...
    await env.DB.prepare(`INSERT INTO caregivers
      ...
      ON CONFLICT(legacy_floot_id) DO UPDATE SET
        ...
        profile_photo_url=excluded.profile_photo_url,source='legacy_carekoya',source_detail='Floot CareKoya profile',work_status='unknown',
        last_confirmed_at=NULL,sms_consent=0,sms_consent_at=NULL,updated_at=CURRENT_TIMESTAMP`)
      .bind(
        ...
        availability.join(", ")||null,Number(p.travelDistanceMiles||0)||null,p.willingToDrive===true?1:0,
        clean(p.profilePhotoUrl,1000)||null,"legacy_carekoya","Floot CareKoya profile"
      ).run();
    imported++;
  }
  const verified=await env.DB.prepare("SELECT COUNT(*) AS count FROM caregivers WHERE source='legacy_carekoya' AND work_status='unknown'").first<{count:number}>();
  return json({ok:true,imported,verified:Number(verified?.count||0)});
}
